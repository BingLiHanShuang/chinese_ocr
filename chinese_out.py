...
def chinese_o(th2, shrinkTwoTimesTranslation_copy, x, y, w, h, cha_num, get_all, is_ABO):#对table_choose.py的接口
	is_first = 1
	minzu_is_right = 0
	return_change = 0
	chinese_num_lim = 0
	...
	if float(blank_counting)/w <= 0.5:#排除空框
		...
		for row_cut in range(0,len(word_b_set)):
			...
			for j in range(3,w-2):#垂直分割
				...
				if (a[j] >= standard and (word_cutting == 1 or word_cut_counting_over == 1)) or (word_cut_counting >= cha_num and word_cutting == 1) or (j == w-3 and word_cut_counting_over == 0):
					...
					if is_ABO == 1:
						...
						limitation = 5
					if abs(word_end_a - word_start_a) >= limitation:#列像素超过5则判断为一个手写字
						...
						if (word_cut_mean[0] < 220 and cha_num == 3) or (word_cut_mean[0] < 245 and cha_num != 3):#排除空框
			...
			if len(word_set) >= cha_num:#当分割个数超过设定的个数则偏向合并，否则切分
				...
				for j in range(0,len(word_set)):
					...
					#print word_average_length
					if word_length_set[j] < word_average_length*0.75:#1.25,0.8
		...
		if is_ABO == 1:#若是在判断血型
			...
			if h_num != 0:
				...
				last_cha_set = chinese_ocr.inference(img, 1)#手写汉字识别，判断血型栏最后一字是否为“型”
			if len(last_cha_set) != 0:
				#print len(last_cha_set)
				if (last_cha_set[0] == 689 or last_cha_set[1] == 689 or last_cha_set[2] == 689):
					last_cha = '型'#若最后一字为“型”，则在血型识别结果末尾加上“型”
					finding_len = len(word_cluster_cut_set) - 1#若最后一字为“型”，ABO识别时不识别最后一位
				else:
					last_cha = ''
			...
			for num in range(0,finding_len):
				...
				img = Image.fromarray(~bin)#OpenCV Mat转PIL
				#img = Image.fromarray(~bin)
				
				ABO_result = ABO_mix.ABO_detection(img)#手写ABO字母识别，判断血型
				print ABO_result
				chinese_result = chinese_result + str(ABO_result)#将识别字符放置在先前识别字段后
		else:
                        #若不是在判断血型栏
			for num in range(0,len(word_cluster_cut_set)):
				img = word_cluster_cut_set[num]
				img_color = word_cluster_color_cut_set[num]
				smaller = 0.9#0.9--normal,0.5--ABO,0.8--jiguan，单字缩小倍数
				...
				if get_all == 1:
					...
					if get_all == 2 and len(word_cluster_cut_set) == 1:
						...
						if is_first == 1 and cha_num == 1:
							is_first = 0#标志是否是识别到的第一个汉字字符
							#判断民族识别首字是否为常见民族首字，若不是则统一按“汉”处理
							if str(chinese) == '汉' or str(chinese) == '蒙' or str(chinese) == '满' or str(chinese) == '朝' or str(chinese) == '回' or str(chinese) == '土':
								minzu_is_right = 1
							else:
								chinese_result = '汉'
						#若民族首字正确，则依次拼接识别结果
						if (minzu_is_right == 1 and cha_num == 1 and chinese_num_lim < 3) or (cha_num != 1 and chinese_num_lim < 3):
							chinese_result = chinese_result + str(chinese)#han meng man chao hui tu
							chinese_num_lim+=1
	if get_all == 1 or return_change != 0:
		return return_change, chinese_set
	if get_all != 1:
		if last_cha == '型':
			chinese_result = chinese_result + '型'#若识别到“型”，则在血型识别结果上拼接“型”字
			return_change = 2
