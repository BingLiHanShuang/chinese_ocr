		...
		for row_cut in range(0,len(word_b_set)):
			...
			for j in range(0,len(word_set)):
				...
				for i in range(word_set[j][0], word_set[j][1]):
					word_a_counting+= a[i]
				print word_a_counting
				if word_a_counting >= 15:#当垂直投影量累计超过15则进入切割数字流程（排除空栏）
			...
			if len(word_cut_save) < num_num:#若分割出的数字个数少于设定的数字个数，则进入切割判断流程
				...
				for j in range(0,len(word_cut_save)):
					rows_word_cut,cols_word_cut = word_cut_save[j].shape
					#print cols_word_cut
					if cols_word_cut > 1.5*word_cut_AverageLength:#若该段长度超过字段平均长度的1.5倍，进入切割流程
			...
			print "len(word_cut_result)",len(word_cut_result)
			
			if len(word_cut_result) == 1:
				h_single, w_single = word_cut_result[0].shape
				print float(w_single)/h_single
			if len(word_cut_result) == 1 and float(w_single)/h_single > 1.78:#若数组中只有一个字段，则通过判断其长宽比，确定是否要平均切分为2个字段
				word_cut_result = []
				c_h = 0
				for j in range(0,2):
					word_cut = word_cut_save[0][0:h_single, w_single*j/2: w_single*(j+1)/2]
					word_cut_result.append(word_cut)
					cv.imwrite('word_cut_half' + str(j) + '_detail' + str(c_h) + '.jpg', word_cut)
					c_h+= 1
			
			row_result = mnist_recognize.mnist_recog(word_cut_result)#调用mnist_recognize.py中的mnist_recog()函数进行手写数字识别
